...
def spectral_clustering(vocab_embeddings, vocab, topics):
    print("Spectral clustering", len(vocab), "words into", topics, "clusters")
    model = SpectralClustering(n_clusters=topics, random_state=2345, affinity = "nearest_neighbors").fit(vocab_embeddings)
    labels = model.labels_
    words = find_words_for_cluster(labels, topics)
    for k in range(len(words)):
        print("Topic", k+1, ":", [vocab[words[k][i]] for i in range(min(len(words[k]), 20))])
    return labels, model


def affprop_clustering(vocab_embeddings, vocab, weights=None, rerank=True):
    print("Affinity propagation clustering of", len(vocab), "words")
    if weights is None:
        aff_prop = AffinityPropagation(random_state=2345).fit(vocab_embeddings)
    else:
        aff_prop = AffinityPropagation(random_state=2345, preference=weights).fit(vocab_embeddings)
    labels = aff_prop.labels_
    centers = aff_prop.cluster_centers_
    n_topics = len(centers)
    top_topic_words = []
    print("---", n_topics, "clusters found ---")
    for k in range(n_topics):
        topk_vals = sort_closest_center(centers[k], labels, vocab_embeddings, k)
        if rerank:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        else:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        print("Topic", k+1, ":", top_words)
        top_topic_words.append(top_words)
    return labels, centers


def kmeans_clustering(vocab_embeddings, vocab, n_topics, weights=None, rerank=True):
    print("Kmeans clustering", len(vocab), "words into", n_topics, "clusters")
    if weights is not None:
        kmeans = KMeans(n_clusters=n_topics, random_state=2345).fit(vocab_embeddings, sample_weight=weights)
    else:
        kmeans = KMeans(n_clusters=n_topics, random_state=2345).fit(vocab_embeddings)
    m_clusters = kmeans.predict(vocab_embeddings, sample_weight=weights)
    centers = np.array(kmeans.cluster_centers_)
    top_topic_words = []
    #indices = []
    for k in range(n_topics):
        topk_vals = sort_closest_center(centers[k], m_clusters, vocab_embeddings, k)
        if rerank:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        else:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        ...
        top_topic_words.append(top_words)
        #print(indices)
    return m_clusters, centers, kmeans
...
#         if rerank:
#             ind, top_words = find_top_words(n_words, topk_vals, vocab)
#         else:
#             ind, top_words = find_top_words(n_words, topk_vals, vocab)
#         #print(indices)
#     return m_clusters
...
def GMM_clustering(vocab_embeddings, vocab,  n_topics, cov_type, rerank=True):
    print("GMM clustering", len(vocab), "words into", n_topics, "clusters")
    print("Covariance type:", cov_type)
    GMM = GaussianMixture(n_components=n_topics, random_state=2345, covariance_type=cov_type).fit(vocab_embeddings)
    means =GMM.means_
    covariances = GMM.covariances_
    labels = GMM.predict(vocab_embeddings)
    for k in range(GMM.n_components):
        density = scipy.stats.multivariate_normal(cov=GMM.covariances_[k], mean=GMM.means_[k]).logpdf(vocab_embeddings)
        topk_vals = density.argsort()[-1*len(density):][::-1].astype(int)
        if rerank:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        else:
            ind, top_words = find_top_words(10, topk_vals, vocab)
        print("Topic", k+1, ":", ", ".join(top_words))
...
#         if rerank:
#             ind, top_words = find_top_words(100, topk_vals, vocab)
#         else:
#             ind, top_words = find_top_words(10, topk_vals, vocab)
#
...
def get_top_words(centers, labels, vocab_embeddings, vocab):
    n_topics = centers.shape[0]
    top_topic_words = []
    for k in range(n_topics):
        topk_vals = sort_closest_center(centers[k], labels, vocab_embeddings, k)
        ind, top_words = find_top_words(10, topk_vals, vocab)
...
def find_top_words(n_words, top_vals, vocab):
    ind = []
    unique = set()
    top_words = []
    for i in top_vals:
        ...
        if word not in unique:
            ...
            if len(unique) == n_words:
                break
    return ind, top_words
