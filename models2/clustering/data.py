...
def get_yle_docs_vocab(filepath, max_docs=50000, min_df=5, max_df=0.8):
    languages = ['fi', 'sv']
    ...
    for lang in languages:
        ...
        if len(docs) > max_docs:
            ...
            sampled_docs = docs
        vectorizer = CountVectorizer(stop_words=stops[lang], min_df=min_df, max_df=max_df)
        counts = vectorizer.fit_transform(sampled_docs)
        vocab = vectorizer.get_feature_names()
        valid_vocab = [w for w in vocab if len(re.findall('[0-9]+', w)) == 0]
        valid_vocab = [w for w in valid_vocab if any(c in w for c in punct) == False]
        valid_vocab = [w for w in valid_vocab if len(w) > 2]
...
def get_yle_docs_vocab_mem_friendly(filepath, max_docs=30000, min_freq=8):
    languages = ['fi', 'sv']
    data = json.load(open(filepath))
    vocab_dict = {}
    docs_dict = {'fi':[], 'sv':[]}
    for lang in languages:
        ...
        for doc in sampled_docs:
            dictionary.extend(doc.split())
        vocab_counts = Counter(dictionary)
        valid_vocab = [word for word in list(vocab_counts.keys()) if vocab_counts[word] >= min_freq]
        valid_vocab = [w for w in valid_vocab if len(re.findall('[0-9]+', w)) == 0]
        # valid_vocab = [w for w in valid_vocab if any(c in w for c in punct) == False]
        valid_vocab = [w for w in valid_vocab if len(w) > 2]
        print(lang.upper(), "vocab:", len(valid_vocab))
        print(valid_vocab[:20])
        vocab_dict[lang] = valid_vocab
        docs_dict[lang] = sampled_docs
    return docs_dict, vocab_dict
...
def get_tf_scores_mem_friendly(docs):
