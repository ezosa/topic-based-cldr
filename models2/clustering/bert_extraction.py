...
argparser.add_argument('--data', default='yle', type=str, help="dataset: wiki, yle or denews")
argparser.add_argument('--data_path', default='./data/yle/', type=str, help="filepath to dataset")
...
argparser.add_argument('--use_stopwords', default=1, type=int, required=False, help="1 for incl stopwords")
argparser.add_argument('--use_full_vocab', default=1, type=int, required=False, help="1 for incl stopwords")
argparser.add_argument('--lang', default='en', type=str, required=False, help="language to extract")
...
class BertWordFromTextEncoder:
    ...
    def load_bert_models(self):
        ...
        pretrained_weights = 'bert-base-multilingual-uncased' #'bert-base-uncased'
    ...
    def eb_dump(self, save_fn):
        ...
        with open(save_fn, 'wb') as f:
            ...
            compound_save = save_fn[:-4]+"_compound_words.txt"
            with open(compound_save, 'w') as ff:
                ff.write("\n".join(list(self.compounds)))
                print(f"{len(self.compounds)} compound words saved to:", compound_save)

            sys.stdout.flush()

    def encode_docs(self, docs=[], agg_by="average", save_fn="", layer=12):
        print("Start BERT extraction for", len(docs), "docs")
        self.agg_by = agg_by
        ...
        if len(save_fn) == 0:
            ...
            print("Save filename provided:", save_fn)
...
def init():
    ...
    from data import get_wiki_docs_vocab, get_yle_docs_vocab, get_denews_docs_vocab
    # if args.use_stopwords == 1:
    #     stopwords = stops[args.lang] #set(line.strip() for line in open("stopwords_en.txt", encoding='utf-8'))
    # else:
    #     stopwords = set()
    ...
    if args.data == 'yle':
        corpus, vocab = get_yle_docs_vocab(args.data_path)
    else:
        corpus, vocab = get_denews_docs_vocab(args.data_path)

    print("Start extraction of BERT embeddings...")
    #for lang in languages[args.data]:
    print("Lang to extract:", args.lang)
    print("Lang:", args.lang.upper())
    valid_vocab = list(vocab[args.lang])
    print("Vocab:", valid_vocab[:50])
    docs = corpus[args.lang]
    #print("Docs:", docs[:2])
    # valid_vocab = [word for word in valid_vocab if word not in punct and vocab[word] >= min_freq]
    ...
    save_path = args.data_path[:-5] + "_bert_extract_"+args.lang+".pkl"
    ...
    encoder.encode_docs(docs=docs, save_fn=save_path, agg_by=args.agg_by, layer=args.nlayer)
